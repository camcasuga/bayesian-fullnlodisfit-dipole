# NLO DIS fit
This repository contains Python and C++ codes to evaluate dipole amplitudes using grids from the Bayesian fit to HERA total inclusive and charm production data of the BK initial condition at full NLO+NLL accuracy published in arXiv:2604.XXXXX
...
The code `python/read_bk.py` prints and plots the dipole amplitude with the BK datafile and rapidity value as input. Example usage for Y = 0:
```bash
python python/read_bk.py data/mv,balsd/median_bk.dat 0
```
which plots the dipole amplitude N as function of r given Y = 0.

Meanwhile, `python/read_bk_allparams.py` loads all BK datafiles in a folder and plots the mean and 2-sigma band N(r) given rapidity Y. Example usage for Y = 0:
```bash
python python/read_bk_allparams.py data/mv,balsd/bks 0
```
...
```c++
const std::string datafile="../data/mv,balsd/median_bk.dat";
// Read data
