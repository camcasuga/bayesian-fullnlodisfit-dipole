...
class TestNlodipole(unittest.TestCase):
    ...
    def test_bal_sd_qs(self):
        ...
        # Test for MV initial condition
        bk_file_dir = "data/mv,balsd/median_bk.dat"
        Y=np.log(1/0.01)
        ...
        # Test for MV gamma initial condtion
        bk_file_median = "data/mvgam,balsd/median_bk.dat"
        interpolator_median = nlodipole.BKDipole(bk_file_median)
        true_qs2_median = 0.185
    ...
    def test_interpolation_boundaries(self):
        ...
        # MVgamma, balsd test
        bk_file_dir = "data/mvgam,balsd/median_bk.dat"
        interpolator = nlodipole.BKDipole(bk_file_dir)
        self.assertTrue(np.isnan( interpolator.N(-1, 2) )) # negative Y = before initial condition
        self.assertTrue(np.isnan( interpolator.N(9999, 2) )) # too large Y
        self.assertAlmostEqual(interpolator.N(1, 99999), 1, places=5) # large r, should return 1
        ...
        # MVgamma, parent dipole test
        bk_file_dir = "data/mvgam,pd/median_bk.dat"
        interpolator = nlodipole.BKDipole(bk_file_dir)
